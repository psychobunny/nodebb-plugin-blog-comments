(function() {
    ...
    var metaType, articleContent, articleTitle;
    for (var i = 0; i < metaEl.length; i++) {
        ...
        if (metaEl[i].getAttribute('property') === 'og:description') {
            articleContent = metaEl[i].getAttribute('content');
        }
        if (metaEl[i].getAttribute('property') === 'og:title') {
            articleTitle = metaEl[i].getAttribute('content');
        }
    }
    ...
    var articleUTitle = btoa(location.pathname);
    ...
    stylesheet.setAttribute("href", pluginURL + '/css/comments-pelican.css');
    ...
    function authenticate(type) {
        ...
        modal = window.open(ourl + "/" + type + "/#blog/authenticate","_blank","toolbar=no, scrollbars=no, resizable=no, width=600, height=675");
    ...
    function normalizePost(post) {
        return post.replace(/href="\/(?=\w)/g, 'href="' + ourl + '/')
                .replace(/src="\/(?=\w)/g, 'src="' + ourl + '/');
    }
    ...
    XHR.onload = function() {
        if (XHR.status >= 200 && XHR.status < 400) {
            ...
            data.relative_path = ourl;
            data.redirect_url = articlePath;
            data.article_id = articleUTitle;
            ...
            if (commentsCategory && data.category) {
                commentsCategory.innerHTML = '<a href="' + ourl + '/category/' + data.category.slug + '">' + data.category.name + '</a>';
            }

            if (commentsAuthor && data.mainPost) {
                commentsAuthor.innerHTML = '<span class="nodebb-author"><img src="' + data.mainPost.user.picture + '" /> <a href="' + ourl + '/user/' + data.mainPost.user.userslug + '">' + data.mainPost.user.username + '</a></span>';
            }
    ...
    function reloadComments() {
        XHR.open('GET', ourl + '/comments/get/' + blogger + '/' + articleUTitle + '/' + pagination, true);
